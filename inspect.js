  ...
  class Inspector {
    ...
    getData(e) {
      ...
      if (e.target.id !== this.contentNode) {
        this.jsonObj       = {};
        this.jsonObj.id    = this.getNullIfEmptyOrUndefined(e.target.id);
        this.jsonObj.name  = this.getNullIfEmptyOrUndefined(e.target.name);
        // Validar si quieren textContent, innerText o innerHTML
        // textContent no retorna el texto <script>, <style> ni elementos ocultos
        this.jsonObj.text  = this.getNullIfEmptyOrUndefined(e.target.innerText);
        this.jsonObj.xPath = this.getNullIfEmptyOrUndefined(this.getXPath(e.target));
        this.jsonObj.class = this.getNullIfEmptyOrUndefined(e.target.class);
        this.jsonObj.css   = this.getNullIfEmptyOrUndefined(this.getCssSelector(e.target));
        this.jsonObj.value = this.getNullIfEmptyOrUndefined(e.target.value);
        this.jsonObj.type  = this.getNullIfEmptyOrUndefined(e.target.type);
        this.jsonObj.tag   = this.getNullIfEmptyOrUndefined(e.target.tagName);
        this.jsonObj.href  = this.getNullIfEmptyOrUndefined(e.target.href);
        this.jsonObj.src   = this.getNullIfEmptyOrUndefined(e.target.src);
        this.jsonObj.style = this.getNullIfEmptyOrUndefined(e.target.getAttribute("style"));
        const jsonStr = JSON.stringify(this.jsonObj);
        const contentNode = document.getElementById(this.contentNode);
        if (contentNode) {
          contentNode.innerText = jsonStr;
        } else {
          const contentHtml = document.createElement('div');
          contentHtml.innerText = jsonStr;
          contentHtml.id = this.contentNode;
          document.body.appendChild(contentHtml);
        }
        this.options.clipboard && ( this.copyText(jsonStr) );
      }
    }

    getNullIfEmptyOrUndefined(value) {
      var output = "";
    ...
    setOptions(options) {
      ...
      this.styles = `*{cursor:crosshair!important;}#xpath-content{${position};cursor:initial!important;padding:10px;background:rgba(233,133,143);color:white;position:fixed;font-size:14px;z-index:10000001;}`;
      this.activate();
    }

    createOverlayElements() {
      const overlayStyles = {
        background: 'rgb(120, 170, 210, 0.7)',
